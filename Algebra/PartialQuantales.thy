theory PartialQuantales
  imports PartialMonoid Quantale
begin
...
class partial_near_quantale = complete_lattice + partial_semigroup +
  assumes partial_qSup_distr: "\<forall>y \<in> Y. y ## x \<Longrightarrow> \<Squnion> Y \<oplus> x = \<Squnion> ((\<lambda>y. y \<oplus> x) ` Y)"

class partial_pre_quantale = partial_near_quantale +
  assumes partial_qSup_subdistl:  "\<forall>y \<in> Y. x ## y \<Longrightarrow> \<Squnion> ((\<lambda>y. x \<oplus> y) ` Y) \<le> x \<oplus> \<Squnion> Y"

class partial_quantale = partial_pre_quantale +
  assumes partial_qSup_supdistl: "\<forall>y \<in> Y. x ## y \<Longrightarrow> x \<oplus> \<Squnion> Y \<le> \<Squnion> ((\<lambda>y. x \<oplus> y) ` Y)" 
begin

lemma partial_qSup_distl: "\<forall>y \<in> Y. x ## y \<Longrightarrow> x \<oplus> \<Squnion> Y = \<Squnion> ((\<lambda>y. x \<oplus> y) ` Y)"
...
begin
sublocale Sup_pmult!: near_quantale _ _ _ _ _ _ _ _ pmult
...
begin
sublocale Sup_pmult!: near_quantale_unital "1'" "op \<oplus>" 
...
sublocale Sup_pmult!: pre_quantale  _ _ _ _ _ _ _ _ pmult
