...
"op +" "op \<odot>" "(1\<Colon>'a)" "(0\<Colon>'a)" "r" "op \<le>" "op <" 
proof 
  fix x y z :: 'a
...
proof
  fix x y z :: 'a
  show "x+x = x"
    by (metis add_comm r1 r3 distrib_right mult_onel
qed
*)
...
"op +" "op \<odot>" "(1\<Colon>'a)" "(0\<Colon>'a)" "ar" "op \<le>" "op <" 
proof 
  fix x y z :: 'a
  show "x \<odot> y \<odot> z = x \<odot> (y \<odot> z)"
    by (metis mult_assoc opp_mult_def)
...
sublocale antirange_semiring < range: range_semiring "op +" "op \<cdot>" "(1\<Colon>'a)" "(0\<Colon>'a)" "r" "op \<le>" "op <" 
proof
  fix x y :: 'a
  have  "x = x\<cdot>r(x)"
   ...
   thus "x+x\<cdot>r(x) = x\<cdot>r(x)"
     by (metis dual.order_refl less_eq_def)
...
"op +" "op \<odot>" "(1\<Colon>'a)" "(0\<Colon>'a)" "ar" "op \<le>" "op <" "star"
proof
  fix x y z :: 'a
...
"op +" "op \<odot>"  "(1\<Colon>'a)" "(0\<Colon>'a)" "ar" "op \<le>" "op <" "bdiamond"
proof
  fix x y :: 'a
...
bbox  "op +" "op \<odot>" "(1\<Colon>'a)" "(0\<Colon>'a)" ar "op \<le>" "op <" bdiamond 
proof
  fix x y :: 'a
...
sublocale bmodal_kleene_algebra \<subseteq> dual!: fmodal_kleene_algebra
 "bbox" "op +" "op \<odot>" "(1\<Colon>'a)" "(0\<Colon>'a)" "ar"  "op \<le>" "op <""bdiamond" "star"
proof
  fix x y z :: 'a
  show "(1\<Colon>'a) + x \<odot> x\<^sup>\<star> \<le> x\<^sup>\<star>"
