...
lemma local_assign: "free_pred v R \<Longrightarrow> local (assign v t) R"
  by (force simp: local_eq basic_local_def assign_def basic_def sep_def valid_mem_def)
...
lemma local_cons [lptran]: "free_pred v R \<Longrightarrow> local (cons u t) R"
apply (simp add: cons_def local_eq basic_local_def)
apply auto
apply (rule_tac someI2_ex)
defer
apply (erule conjE)
apply (subst (asm) someI_ex) 
oops
...
lemma hl_assign [sl]: "P \<subseteq> subst_pred Q z_upd t \<Longrightarrow> ht P (assign z_upd t) Q"
  by (auto simp: assign_def valid_mem_def intro!: sl)
...
lemma sl_mut [sl]: "ht ((e \<mapsto> -) * ((e \<mapsto> e') -* r)) (mutation e e') r"
...
lemma sl_lookup [sl]: "ht (EXS v'. (e \<mapsto> v') * ((e \<mapsto> v') -* (subst_pred r v_upd v'))) (lookup v_upd e) r"
